...
Datastore hanterar datalagring med både SQL och NoSQL. För den som inte är bekant med NoSQL så är det en alternativ design för databaser som skiljer sig från SQL-databaser \cite{nosql}. NoSQL är designad på så sätt att det inte bygger på tabeller som i vanliga relationsdatabaser. Marknaden som existerar idag är vad som kunde kallas för en \textit{Big Data Reality} som kännetecknades av den enorma mängden data som måste hanteras av dagens databaser. Designen utvecklades för att möta kraven som ställdes av dagens så kallade \textit{Big Data Reality}. NoSQL utvecklades för att bidra med en enklare design och lättare tillgänglighet. Det är mest datastrukturerna som skiljer NoSQL från vanliga relationsdatabaser. Detta leder till att vissa operationer är lättare och mer effektiva att utföra i NoSQL medan andra är snabbare i vanliga relationsdatabaser \cite{nosqlfacts}. NoSQL används alltmer av applikationer som måste hantera stora mängder data men också web-system som ska exekveras i realtid \cite{nosqlcloud}. Egenskaperna som NoSQL besitter passar för vår applikationen då ett tak för antalet användare inte var satt samt att vi önskade att exekvera i realtid.
...
\begin{itemize}
\item Streamlined cognitiv walkthrough (SCW)
\item Think aloud utvärdering
\item Heurisitc evaluation (HE)
\end{itemize}

\subsection{Utvärderingar och deras användbarhet}
Utvärderingar är nyttiga verktyg för att hitta problem eller för att hitta områden som är lämpade för vidare utveckling \cite{benyon2010designing}. Utvärderingarna vi har valt att utföra är till för att granska produkten vi har utvecklat för att se om den möter kraven vi ställer på den. Vi hoppades att utvärderingarna skulle ge oss en bild av applikationens användbarhet och effektivitet samt se till att den var tillmötesgående för användaren. Ur en designsynpunkt var vi intresserade av att se om applikationen fyller sitt syfte, om den är rolig samt om den engagerar användaren.

\subsection{Kognitiv Genomgång}
En \textit{Kognitiv Genomgång} \cite{cw} är en utvärderingsmetod som är till för att avgöra problem med användbarhet i ett interaktivt datorsystem. Utvärderingen som vi valt att utföra är en nyare version av det gamla konceptet.
Utvärderingen går till på så sätt att systemets utvecklare väljer en grupp med användare som aldrig använt systemet tidigare. Innan dessa personer kan väljas måste systemets utvecklare först analysera vilken typ av användargrupp som systemet riktar sig till. Ett exempel på en fråga som är hjälpsam vid ett sådant urval är exempelvis: "Vilken typ av bakgrundskunskap behövs för att använda systemet?".\\ 
När användarna för utvärderingen är valda ska systemets utvecklare sedan välja ett antal vitala uppgifter som användarna skall utföra i systemet. Vitala uppgifter kan var sådana som användare i systemet måste utföra ofta, uppgifter som inte används ofta men som fortfarande anses kritiska för systemet eller sådana uppgifter som visar på systemets duglighet.
Varje uppgift har ett antal tillhörande frågor som utvecklarna ställer sig för att få en bättre förståelse av hur dessa uppgifter kan exekveras bättre.
Exempel på frågor till varje vald uppgift är: \\
Är det troligt att användarna utför "rätt" handling för varje uppgift? Med andra ord: förväntar sig utvecklarna att användarna ska göra just den handlingen vid uppgiften?\\
Om användaren gjort "rätt" val i uppgiften, vet de då att detta var rätt val? Får användaren någon feedback som tyder på detta?\\
...
\subsubsection{Kognitiv Genomgång av Chronos} 
Uppgifterna vi valde att använda i den kognitiva genomgången var:


\subsection{Think Aloud}
\textit{Think Aloud} \cite{thinkaloud} är en utvärderingsmetod som går ut på att användare ska utföra ett antal uppgifter som utvecklarna har valt. Det är sedan användarnas jobb att under tiden uppgifterna utförs att säga vad de tänker på. Utvecklarna antecknar det användarna säger kontinuerligt under tiden uppgifterna utförs. Anteckningarna analyseras sedan av utvecklarna för att se vilka delar som eventuellt var för svåra, dåliga eller rent av ineffektiva. Designen av systemet kommer sedan att alterneras eller skapas på nytt beroende på inputen som gavs från användarna.\\
...
En \textit{Heuristic Evalutation} \cite{benyon2010designing} är en utvärderingsmetod för att fastställa problem i användbarhet hos ett system, problemens storlek samt vart de finns.
...
En stor del av utvärderingsgruppen var elever från Civilingenjörsprogrammet - IT på Uppsala Universitet som alla hade gått en kurs i Människa och Datorinteraktion. Vi valde även att inkludera en person som hade gått en separat utbildning i Apputveckling och grafisk design. 
...
Gränssnittet och användandet av applikationen skulle också utvärderas med hjälp av \textit{Think Aloud } \cite{thinkaloud} och en \textit{Heuristic Evaluation } \cite{heuristic}.
