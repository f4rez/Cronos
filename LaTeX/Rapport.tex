...
\section{Bakgrund}
I detta stycke beskrivs ämnet som utvecklingen av vår app är starkt kopplat till. Bland annat beskrivs 
...
\subsection{Balanseringsteori}
		%%-----------------------------------------------------------%%
		%%															%%
		%%		EJ KORREKTURLÄST FÖR KONTROLL AV TEMPUS				%%
		%%															%%
		%%-----------------------------------------------------------%%
En viktig aspekt i det här projektet var att rangordna spelare i förhållande till varandra för att med hjälp av det skapa mer rättvisa matcher. Den här problematiken fanns det redan många som hade funderat på och de flesta större spel använde sig också av något system för detta. Två etablerade system var Elo och Glicko, varav Elo var det som implementerades i det här projektet.

Elo var ett system som förutsåg hur stor del av en mängd spelade matcher som borde ha vunnits av vardera spelare, baserat på en den svårighetsnivå respektive spelare hade. Det här systemet justerade, efter en match, poängen enligt beskrivningen ovan. Eftersom en lägre rankad spelare fick fler poäng vid vinst mot en högre rättade systemet på sikt sig självt, vilket innebar att spelarna faktiskt fick den nivå de förtjänade.

Elo-systemet fungerade enligt följande. Varje spelare fick en initial Elo-poäng på 1000. Denna justerades sedan beroende på utfallet av en match i förhållande till det förväntade utfallet. Det förväntade utfallet beräknades:

$$E_A = \frac{1}{1+10^{(R_B-R_A)/400}}$$

Där $E_A$ var den förväntade andelen matcher som spelare A borde vinna. Är värdet till exempel 0.84 så innebär det att 84\% av matcherna borde vinnas av spelare A enligt nuvarande Elo-poäng. $R_B$ är spelare B:s nuvarande Elo-poäng och $R_A$ är spelare A:s nuvarande poäng.
...
\section{Diskussion}
Diskutera kring hur vi hade kunnat utveckla på ett bättre sätt samt diskutera lite mer kring om vi hade kunnat välja andra hjälpmedel etc (för att det skulle ha blivit mer effektivt etc).
