...
hämtare, ett balanseringssystem och en databas. Informationshämtaren hanterar den semi-automatiska genereringen av händelser och balanseringssystemet justerar svårighetsgraden på spelarna samt händelserna. Balanseringen sker för att maximera spelupplevelsen och för att hålla kvar spelarna så länge som möjligt.
...
Sidor som visade sig vara enkla att läsa in var Wikipedias undersidor av typen listningssidor. De här sidorna var lätta att läsa in eftersom de var väldigt strukturerade. Händelserna var redan sammanfattade i en mening vardera och intill dessa stod det alltid ett årtal. Varje mening och årtalskombination var sedan en enhet i en större punktlista. Vad för beskrivning som finns relaterat till året beror på vilken typ av listsida som har valts. Ett exempel är sidan som listar datorspelshändelser efter år som illustreras i Figur 2 nedan.

Parsern använder sig av ett bibliotek i go som kan läsa av html- och xml-kod. Wikipediaartiklarna som läses in är alltid konsekventa med sin html-struktur och därför går det bra att bygga en parser för artiklarna.
Strukturen gjorde det enkelt att vid inläsning av en sida separera varje $<$li$>$-element där ett bindestreck eller kolon förekom och därefter spara varje kombination av årtal och händelse i ett temporärt cache.
...
Chronos är uppbyggd av tre olika Activities~\cite{activities} som vi har döpt till LoginActivity, MainActivity och MatchActivity. När Chronos öppnas så startas MainActivity som undersöker om användaren är inloggad. Om användaren inte är inloggad kommer en LoginActivity att startas varpå MainActivity pausas i väntan på ett resultat från LoginActivity.

LoginActivity är en väldigt simpel webbläsare som vid uppstart öppnar en inloggningssida som tillhandahålls av vår server. När användaren har loggat in skickas ett resultat tillbaka till MainActivity om att hen är inloggad och att den kan fortsätta med uppstart av applikationen.

MainActivity är uppbyggt av flera fragment~\cite{fragment} där det viktigaste är StartPageFragment. StartPageFragment är fragmentet som startas som standard av MainActivity. När detta fragment startas skickar det ett meddelande till servern för att kolla om användaren är registrerad i vår databas. När fragmentet får tillbaka ett svar skickas ett nytt meddelande till servern som efterfrågar användarens aktiva samt avslutade matcher. Svaret från servern är ett JSON-objekt~\cite{jsonobject} som vi omvandlar till en ArrayList~\cite{arraylist} av vanliga java objekt. Listan skickas in till en anpassad ArrayAdapter~\cite{arrayadapter} som kan ta till vara på informationen som finns i objekten. Detta är möjligt genom att vi har skrivit en egen implementation av metoden getView~\cite{getview}. Varje objekt i listan skickas genom den och tillbaka fås en vy som läggs till i en lista som syns i fragmentet. Alla listor och rutnät som finns i Chronos är uppbyggt enligt samma princip med egna implementationer av ArrayAdapters med olika objekt som vi skapar genom JSON-data som vi får från vår server.  

Android har satt restriktionen att man inte får göra några nätverksanrop på huvudtråden som applikationen körs på. Så för all vår nätverkskommunikation använder vi AsyncTasks~\cite{asynctask} som körs på en egen tråd. Vi har skapat en anpassad AsyncTask som bland annat tar emot de parametrar som behövs för att serven ska veta vad vi eftersöker. Förutom detta tar den även emot vilket fragment eller aktivitet som har skapat och startat anropet. När AsyncTask exekveras anropar den olika funktioner beroende på vilka parametrar som skickas med, alla dessa funktioner är uppbyggda på samma sätt. De olika funktionerna som kan anropas skapar först en URL som leder till rätt adress på servern, sedan hämtas en cookie från en cookiestore~\cite{cookiestore} som används av servern för att validera vem som gör anropet. Med hjälp av HttpURLConnection~\cite{httpurlconnection} öppnas sedan en anslutning till serven. Anslutningen läses av och byggs ihop till en sträng som returneras till AsyncTask. Beroende på vilket fragment eller vilken aktivitet som anropat AsyncTask görs sedan ett callback till den med JSON-strängen som argument.
