...
Matchvyn kan lättast beskrivas som startsidan i hela applikationen. Det är här användaren ser vilka matcher hen har pågående eller har avslutat. I denna vy kan Användaren kan även starta en ny slumpad match samt söka efter en spelare eller vänner att utmana. 
Det finns fyra olika handlingar som användaren kan göra här vid denna sida. De kan klicka på hamburgarmenyn ($\equiv$) uppe i vänstra hörnet för att öppna navigationsvyn. De kan även klicka på en av de matcher de har pågående för att spela en ny omgång eller titta på matchens tillhörande matchstatistik. Om använder väljer att klicka på en av matcherna skickas hen till matchstatistikvyn (3).
\begin{figure}[H]
\begin{center}
...
Hit kommer användaren när de klickar på en match eller de har spelat färdigt en omgång. Vyn består av de två användare som möter varandras profilbild och namn och under finns varje resultatet för varje runda som spelats. Längst ner finns två olika element, i mitten visas den sammanlagda poängen för hela matchen och till höger finns det en FAB som leder till en match(4)  
...
\pagebreak
 \large \textup{4. Match}\\
Här är vyn där matcherna kommer spelas. Överst i vyn finns ett kort som innehåller den händelse som ska placeras in i tidslinjen. Under kortet ligger tidslinjen, den ska sorteras i kronologisk ordning där den tidigaste händelsen ligger högst upp. När matchen startas ligger det en given händelse i tidslinjen och ovanför ligger det en delare (Placera in händelsen i ordning) som representerar händelsen som är skriven på kortet. Delaren är mobil och för att flytta den får man göra ett långt klick och sedan dra den den till den position man vill. Det finns även här en FAB som rättar det placeringen av delaren, om svaret är rätt omvandlas delaren till händelsen som var på kortet det läggs in en ny delare och en ny händelse visas på kortet. Om svaret däremot var fel blinkar delaren till i rött och visar sedan årtalen på alla händelser som är utlagda i tidslinjen.
...
Det finns två sätt att komma till en profilsida. Man kan genom Min profil (10) klicka på en vän få se deras profilsida eller genom Sök spelare (8). Vyn består av en profilbild som är tagen från användarens Google konto, längs kanten på profilbilden ligger det en FAB som vid tryck vecklas ut till ett verktygsfält där man kan utmana profilen (Utmana), lägga till profilen som en vän (Lägg till som vän) eller ta bort profilen från sin vän lista (Ta bort vän). Under profilenbilden visas den andres användaren namn och cirkeldiagram över hur många matcher som hen har vunnit, förlorat och spelat oavgjort. Klickar man till exempel på den del som visar hur många man förlorat visas även hur många matcher det är man har förlorat. 
