...
Att tävla är både en rolig och en social aktivitet. Oavsett om målet är att vinna eller bara ha kul så bidrar spel ofta med någon form av tävlingskänsla. Att kunna få denna tävlingskänsla och samtidigt en källa för nöje så förmånligt i sin telefon är något som vi tror är något att eftersträva i mobila spel. Spel som exempelvis Quizkampen \cite{quiz} och Wordfeud \cite{wordfeud} besitter båda denna egenskap och det är därför vi tror att dessa spel har blivit så populära på marknaden \cite{appsalesrating}. Vi som utvecklare finner att Quizkampen och Wordfeud har lyckats väl med att designa spel som är enkla, roliga, estetiskt vackra, fria från fel samt effektiva i den meningen att man som spelare fokuserar på det vitala. Vi eftersträvar att skapa ett spel som besitter samma egenskaper och som kan framkalla samma tävlingskänsla som i dessa spel. Ett spel som går ut på att ordna historiska händelser relativt till andra händelser. 
...
\subsubsection{Faktorer som gör VÅRT SPEL unikt i förhållande till konkurrenterna}
...
Google App Engine används lokalt för att simulera serverapplikationen. I skarpt läge kommer App Engine även användas, men då i Googles moln. App engine är ett värdefullt verktyg i vår utveckling med den enkla förklaringen att de tjänster som erbjuds när man utvecklar och körs sina applikationer via denna motor är otroligt värdefulla. Det är bland annat lätt att underhålla applikationen och skala när trafiken och datan kräver det  \cite{googleappengine}. För att läsa mer om App Engine se \ref{Google App Engine}. \\
...
Datastore hanterar datalagring med både SQL och NoSQL. För den som inte är bekant med NoSQL så är det en alternativ design för databaser som skiljer sig från SQL-databaser \cite{nosql}. NoSQL är designad på så sätt att det inte bygger på tabeller som i vanliga relationsdatabaser. Designen skapades för att möta kraven som idag ställs på datahantering. Dagens marknaden existerar idag i vad som kan kallas för en \textit{Big Data Reality} som kännetecknas av den enorma mängden data som måste hanteras. NoSQL utvecklades för att bidra med en enklare design, lättare tillgänglighet och horisontell skalning. Det är mest datastrukturerna som skiljer NoSQL från vanliga relationsdatabaser. Detta leder till att vissa operationer är lättare och mer effektiva att utföra i NoSQL och andra är snabbare i vanliga relationsdatabaser. NoSQL används alltmer av applikationer som måste hantera stora mängder data men också web-system som opererar i realtid \cite{nosqlfacts}. Detta passar ganska bra in på vår applikation då ett tak för antalet användare inte är satt samt att vi önskar att operera i realtid.

Med Datastore behöver utvecklarna av applikationen inte oroa sig över hur sökning och borttagning sker i databasen. Detta kan förklaras med faktumet att Datastore inte använder sig direkt av schemas som en relationsdatabas i exempelvis SQL. Sökning i databasen är också effektivt tack vare den solida sökmotorn som biblioteket tillhandahåller.
...
Sidor som har visat sig vara enkla att läsa in är Wikipedias undersidor av typen listningssidor. Den här typen av sidor har en punktlista med årtal samt en beskrivande mening av det årtalet inom den kategori som Wikipediasidan handlar om. Ett exempel är sidan som listar datorspelsår: Som återfinns vid denna  \textbf{\href{http://sv.wikipedia.org/wiki/Lista_\%C3\%B6ver_datorspels\%C3\%A5r}{länk.}} 
Denna struktur gör det enkelt att vid inläsning av en sida separera varje $<$li$>$-element där ett bindestreck eller kolon förekommer och därefter spara varje kombination av årtal och händelse i ett temporärt cache.

\begin{figure}[H]
	\begin{centering}
	\includegraphics[width=\textwidth]{Listsida-wikipedia} 
	\end{centering}
