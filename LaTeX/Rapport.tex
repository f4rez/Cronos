...
Chronos är uppbyggd av tre olika Activities som vi har döpt till LoginActivity, MainActivity och MatchActivity. När Chronos öppnas så startas MainActivity som undersöker om användaren är inloggad. Om användaren inte är inloggad kommer en LoginActivity att startas varpå MainActivity pausas i väntan på ett resultat från LoginActivity.
...
MainActivity är uppbyggt av flera fragment där det viktigaste är StartPageFragment. StartPageFragment är fragmentet som startas som standard av MainActivity. När detta fragment startas skickar det ett meddelande till servern för att kolla om användaren är registrerad i vår databas. När fragmentet får tillbaka ett svar skickas ett nytt meddelande till serven som efterfrågar användarens aktiva samt avslutade matcher. Svaret från servern är ett JSON-objekt som vi omvandlar till en ArrayList av vanliga java objekt. Listan skickas in till en anpassad ArrayAdapter som kan ta till vara på informationen som finns i objekten. Detta är möjligt genom att vi har skrivit en egen implementation av metoden getView. Varje objekt i listan skickas genom dem och tillbaka fås en vy som läggs till i en lista som syns i fragmentet. Alla listor och rutnät som finns i Chronos är uppbyggt enligt samma princip med egna implementationer av ArrayAdapters med olika objekt som vi skapar genom JSON-data som vi får från vår server.  
